...
// One Chart instance per canvas: a query can render several variables at once, each
// in its own panel.
const charts = new Map();

export function getChart(canvas) {
...
export function readTheme() {
  ...
  return {
    ...
    tooltipBg: v('--tooltip-bg', '#1a1a19'),
    tooltipText: v('--tooltip-text', '#ffffff'),
  };
}
...
/** Internal: renderHistogram replaces a canvas's chart in place. */
  ...
  const chart = new Chart(canvas.getContext('2d'), {
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          // Both colours come from tokens: Chart.js defaults its text to white, so
          // setting only the background breaks as soon as the theme inverts.
