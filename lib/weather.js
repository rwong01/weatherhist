...
async function loadYear({ latitude, longitude, variable, startMD, endMD, year, model, signal }) {
  // The model is part of the key: the same year fetched under a different model is
  // a different number, and must not be served from another model's entry.
  const key = cache.makeKey({ latitude, longitude, variableId: variable.id, startMD, endMD, year, model });
  ...
  const body = await getJSON(
    archiveURL({ latitude, longitude, start_date, end_date, hourly: variable.hourly, model }),
    { signal }
  );
...
export async function fetchWindows({
  ...
  const tasks = variables.flatMap((variable) => years.map((year) => ({ variable, year })));
  let done = 0;
  onProgress({ done, total: tasks.length });

  const loaded = await mapLimit(tasks, CONCURRENCY, async ({ variable, year }) => {
    const result = await loadYear({
      latitude, longitude, variable, startMD, endMD, year, model: model.id, signal,
    });
    onProgress({ done: ++done, total: tasks.length });
    return { variableId: variable.id, ...result };
  });

  const datasets = variables.map((variable) => {
    const perYear = loaded.filter((entry) => entry.variableId === variable.id);
