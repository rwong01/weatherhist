 ...
 *
 * `signal` lets a caller abort a request that a later keystroke has superseded;
 * AbortError propagates rather than being wrapped, so callers can ignore it.
 */
export async function geocode(query, { count = 5, signal } = {}) {
